...
def evaluate_model(model_type, train, test_or_future):
    try:
        if model_type == "Prophet":
            ...
            model.load_state_dict(torch.load("./models/lstm_model.pth"))  # Load pre-trained
            model.eval()
...
def predict_from_file(file_path, date_from=None, date_to=None, period=0):
    data = pd.read_csv(file_path, parse_dates=["point_timestamp"], dayfirst=True)
    ...
    if period > 0:
        ...
        forecast, mape = evaluate_model(best_model, train, test)
    return {
...
if __name__ == "__main__":
    result = predict_from_file("./data/daily.csv", period=0)
    print(f"Best Model: {result['best_model']}")
    print(f"MAPE: {result['mape']:.4f}" if result['mape'] else "MAPE: N/A")
